...
[![CI](https://github.com/savvaskoualis/postern/actions/workflows/ci.yml/badge.svg)](https://github.com/savvaskoualis/postern/actions/workflows/ci.yml)
...
Postern is the missing piece, not a reimplementation of the client:
...
1. Postern binds an HTTP listener on `127.0.0.1:8020` (`saml_port`) — the redirect
...
5. Postern exchanges that id at `/remote/saml/auth_id?id=…` for the `SVPNCOOKIE`
...
On macOS and Linux, Postern therefore installs a small root-owned helper,
`/usr/local/libexec/postern-tunnel`, with a sudoers rule in `/etc/sudoers.d/postern`
scoped to that one path:

```
<you> ALL=(root) NOPASSWD: /usr/local/libexec/postern-tunnel
```
...
`POSTERN_OPENCONNECT`. The full reasoning is in the `THREAT MODEL` blocks at the top of
[`scripts/install.sh`](scripts/install.sh#L19-L35) and
[`scripts/postern-tunnel`](scripts/postern-tunnel#L21-L41). Read them before you widen
anything.
...
- **openconnect**, with Fortinet protocol support. The installers pull it in for you
  ...
  needs openconnect 8.10 or newer; Postern's output parsing was verified against 9.21,
...
`POSTERN_GATEWAY` is required on a first install — Postern ships with no default
...
```sh
POSTERN_GATEWAY=vpn.example.com:10443 \
POSTERN_RELEASE_URL=https://github.com/savvaskoualis/postern/releases/download/v0.1.0/postern-darwin-arm64 \
  bash scripts/install.sh
```

You still need the checkout (or at least `scripts/`) either way: the privileged helper
is installed from `scripts/postern-tunnel`.
...
```sh
xattr -d com.apple.quarantine ~/Downloads/postern-darwin-arm64
```

Or right-click the binary in Finder and choose **Open** once, which records your
consent. `curl` does not set the attribute, so the `POSTERN_RELEASE_URL` path above is
unaffected.

### Windows

Download `postern-windows-amd64.exe` from a release (or run `make release` and take it
from `dist\`), put it next to `scripts\install.ps1`, then from an **elevated**
PowerShell:

```powershell
$env:POSTERN_GATEWAY = "vpn.example.com:10443"
.\scripts\install.ps1
```

If `POSTERN_GATEWAY` is unset the script prompts for it. It installs to
`%ProgramFiles%\postern`, writes `%APPDATA%\postern\config.json`, installs openconnect
via winget if it is missing, and creates a logon scheduled task named `Postern` with
...
| OS | `config.json` and `postern.log` |
| --- | --- |
| macOS | `~/Library/Application Support/postern/` |
| Linux | `~/.config/postern/` (or `$XDG_CONFIG_HOME/postern/`) |
| Windows | `%APPDATA%\postern\` |

The file is optional: any key you omit takes its default. It is created mode `0600` in
a `0700` directory. Postern reads it at startup only, so restart the app after editing.
...
| `helper_path` | `"/usr/local/libexec/postern-tunnel"` | the privileged helper. The sudoers rule is scoped to exactly this path, so changing it without reinstalling makes `sudo` prompt and the tunnel fail to start. |
...
```sh
sudo -n /usr/local/libexec/postern-tunnel stop
```
...
connected. Please
[file an issue](https://github.com/savvaskoualis/postern/issues) with your
`openconnect --version` output and the relevant lines from `postern.log`, so the marker
can be added.
...
```sh
sudo /usr/local/libexec/postern-tunnel stop
```
...
```sh
launchctl bootout gui/$(id -u)/io.github.savvaskoualis.postern 2>/dev/null
rm -f ~/Library/LaunchAgents/io.github.savvaskoualis.postern.plist
sudo rm -f /usr/local/bin/postern /usr/local/libexec/postern-tunnel /etc/sudoers.d/postern
rm -rf ~/Library/Application\ Support/postern
```

### Linux

```sh
rm -f ~/.config/autostart/postern.desktop
sudo rm -f /usr/local/bin/postern /usr/local/libexec/postern-tunnel /etc/sudoers.d/postern
rm -rf ~/.config/postern
```

Quit the tray app first. If you set `POSTERN_HELPER_DIR`, remove the helper from there
instead. `/var/run/postern-openconnect.pid` does not survive a reboot; run the helper's
...
```powershell
schtasks /Delete /TN "Postern" /F
Remove-Item -Recurse -Force "$env:ProgramFiles\postern"
Remove-Item -Recurse -Force "$env:APPDATA\postern"
```
...
```sh
make build     # go build -o postern ./cmd/postern
make test      # go vet ./... && go test -race ./...
make release   # cross-build all four release binaries into dist/
make install   # bash scripts/install.sh on this machine
```

`make release` produces `postern-darwin-arm64`, `postern-darwin-amd64`,
`postern-linux-amd64` and `postern-windows-amd64.exe`. The darwin slices need cgo
